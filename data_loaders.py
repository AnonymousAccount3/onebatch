...
def load_phishing():
    name = "phiusiil+phishing+url+dataset/PhiUSIIL_Phishing_URL_Dataset.csv"
    path = os.path.join(folder, name)
...
def load_optical_radar():
    name = "crop+mapping+using+fused+optical+radar+data+set/WinnipegDataset.txt"
    path = os.path.join(folder, name)
    df = pd.read_csv(path)
    X = df.values.astype(np.float32)
    X /= (np.abs(X).max(0, keepdims=True) + EPS)
